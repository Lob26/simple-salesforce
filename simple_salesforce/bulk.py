...
class SFBulkType:
    ...
    def _get_batch_request_with_batch_results(
        self,
        job_id: str,
        batch_id: str,
    ) -> Iterable[Any]:
        ...
        batch_request = call_salesforce(
            url=url, method="GET", session=self.session, headers=self.headers
        )

        batch_result = self._get_batch_results(
            job_id, batch_id, operation="batch_results"
        )

        results = []
        for idx, i in enumerate(batch_result):
            flattened_request_dict = [
                {k + "." + list(v.keys())[0]: v.get(list(v.keys())[0])}
                if isinstance(v, dict)
                else {k: v}
                for k, v in batch_request.json()[idx].items()
            ]
    ...
    def worker(
        ...
        if not bypass_results:
            ...
            if include_detailed_results:
                ...
                result = self._get_batch_results(
        ...
        * batch_size -- number of records to assign for each batch in the job
        ...
        if operation not in ("query", "queryAll"):
            ...
            with concurrent.futures.ThreadPoolExecutor() as pool:
                ...
                if batch_size == "auto":
                    batches = self._add_autosized_batches(
                        job=job["id"], data=data, operation=operation
                    )
                else:
    ...
    def query(
        self, data: BulkDataStr, lazy_operation: bool = False, wait: int = 5
    ) -> Iterable[Any]:
        """bulk query"""
